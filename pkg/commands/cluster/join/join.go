...
func getFilesFromDestCluster(options *JoinOptions, tmpDir string) (string, error) {
	...
	localTemplatePath := filepath.Join(tmpDir, filepath.Base(ignition.KeepAlivedConfigPath))
	cc.FilePaths = append(cc.FilePaths, kubectl.FilePath{RemotePath: "/hostroot" + ignition.KeepAlivedConfigPath, LocalPath: localTemplatePath})
...
func copyHAFilesToNode(options *JoinOptions, bindPort uint16, altPort uint16, virtualIP string, netInterface string) ([]string, error) {
	...
	for _, file := range assets.Files {
		// Skip keepalived config.  This is generated by an in-cluster
		// service and doesn't need to be seeded.  Plus, it could
		// cause the node to drop its VIP from the source cluster, which
		// will impact reachability for futher steps in the process.
		// Same goes for nginx config.
		if file.Path == ignition.KeepAlivedConfigPath ||
		   file.Path == ignition.NginxConfigPath {
			continue
		}
