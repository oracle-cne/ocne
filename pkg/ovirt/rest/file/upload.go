...
func UploadFile(ovcli *ovclient.Client, url string, reader io.Reader, totalLen int64) error {
	const defChunkSize = 10 * 1024 * 1024
	var chunkSize int64 = defChunkSize
	var start int64
	var end int64 = -1

	remainingLen := totalLen
	for remainingLen > 0 {
		if chunkSize > remainingLen {
			chunkSize = remainingLen
		}
		...
		if err := uploadChunk(ovcli, url, reader, chunkSize, start, end, totalLen); err != nil {
			return err
		}
	}
...
func uploadChunk(ovcli *ovclient.Client, url string, reader io.Reader, chunkLen int64, start int64, end int64, totalLen int64) error {
	// TEMP - until i figure out how to prevent http
