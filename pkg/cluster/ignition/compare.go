			...
			if unitA.Name == unitB.Name {
				...
				for _, dropA := range unitA.Dropins {
					...
					for j, dropB := range unitB.Dropins {
						...
						if dropA.Name == dropB.Name {
							if dropA.Contents == nil && dropB.Contents != nil {
								log.Debugf("Systemd.Unit.Dropin %s in %s has new content", dropA.Name, unitA.Name)
								return false
							} else if dropA.Contents != nil && dropB.Contents == nil {
								log.Debugf("Systemd.Unit.Dropin %s in %s has no content", dropA.Name, unitA.Name)
								return false
							}
							if (dropA.Contents != nil && dropB.Contents != nil) && (*dropA.Contents != *dropB.Contents) {
								log.Debugf("Systemd.Unit.Dropin %s in %s changed content from\n%s\nto%s", dropA.Name, unitA.Name, dropA.Contents, dropB.Contents)
								return false
							}
							matchedD[j] = true
							found = true
					...
					if !found {
						log.Debugf("Systemd.Unit.Dropin %s in %s missing", dropA.Name, unitA.Name)
						return false
					}
				}
			}
